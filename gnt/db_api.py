...
from .models import Profile, Drink, DrinkName, ProfileToLikedDrink, ProfileToDislikedDrink
...
def like_drink(request):
    try:
        ...
        if ProfileToLikedDrink.objects.filter(profile_FK=profile, drink_FK=drink):
            response = {
                'message': "Drink " + str(request.POST['drink_id']) + " has already been liked by " + str(username) + ". No changes to db",
                'status': 422
            }
        else:
            new_like = ProfileToLikedDrink(
                profile_FK=profile, drink_FK=drink)
            new_like.save()
            msg = "Drink " + \
...
def dislike_drink(request):
    try:
        ...
        if ProfileToDislikedDrink.objects.filter(profile_FK=profile, drink_FK=drink):
            response = {
                'message': "Drink " + str(request.POST['drink_id']) + " has already been disliked by " + str(username) + ". No changes to db",
                'status': 422
            }
        else:
            new_dislike = ProfileToDislikedDrink(
                profile_FK=profile, drink_FK=drink)
            new_dislike.save()
            msg = "Drink " + \
                str(request.POST['drink_id']) + "added to " + \
